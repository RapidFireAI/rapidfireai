...
update_readme_md_file() {
    local NEW_VERSION="$1"
    # Update BUILD.md if it has a version comment
    if grep -q "^# RapidFire AI" "$PROJECT_ROOT/README.md" 2>/dev/null; then
        print_info "Updating README.md version comment and wheel version..."
        if [[ "$OSTYPE" == "darwin"* ]]; then
            sed -i '' "s/^# RapidFire AI .*/# RapidFire AI $NEW_VERSION/" "$PROJECT_ROOT/README.md"
        else
            sed -i "s/^# RapidFire AI .*/# RapidFire AI $NEW_VERSION/" "$PROJECT_ROOT/README.md"
        fi
    fi
...
update_readme_md_file "$NEW_VERSION"
...
if ! is_github_actions; then
    ...
    print_info "To deploy to PyPI, merge with main,create tag, create release notes, and create a release"
else
