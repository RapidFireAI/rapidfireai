...
# RapidFire AI Evals Multi-Service Startup Script
# This script starts MLflow server, Evals API server, and frontend tracking server
...
RF_PID_FILE="${RF_PID_FILE:=$HOME/rapidfire_evals_pids.txt}"
...
# Reuse fit frontend for now (unified dashboard)
...
setup_python_env() {
    print_status "Setting up Python environment for Evals..."
...
cleanup() {
    print_warning "Shutting down Evals services..."
    ...
    print_success "All Evals services stopped"
    exit 0
}
...
start_mlflow() {
    print_status "Starting MLflow server for Evals..."
...
# Function to start Evals API server
start_api_server() {
    print_status "Starting Evals API server with Gunicorn..."

    # Check if dispatcher directory exists
    if [[ ! -d "$DISPATCHER_DIR" ]]; then
        print_error "Evals Dispatcher directory not found at $DISPATCHER_DIR"
        return 1
    fi

    # Check if gunicorn config file exists
    if [[ ! -f "$DISPATCHER_DIR/gunicorn.conf.py" ]]; then
        print_error "gunicorn.conf.py not found in evals dispatcher directory"
        print_error "Please create $DISPATCHER_DIR/gunicorn.conf.py"
        return 1
    fi
    ...
    print_status "Evals API server logs will be written to: $RF_LOG_PATH/evals_api.log"
    gunicorn -c gunicorn.conf.py > "$RF_LOG_PATH/evals_api.log" 2>&1 &

    local api_pid=$!
    cd "$SCRIPT_DIR"  # Return to original directory
    echo "$api_pid Evals_API_Server" >> "$RF_PID_FILE"

    # Wait for API server to be ready - use longer timeout for API server
    if wait_for_service $RF_API_HOST $RF_API_PORT "Evals API server" 60; then
        print_success "Evals API server started (PID: $api_pid)"
        print_status "Evals API server available at: http://$RF_API_HOST:$RF_API_PORT"
        return 0
    else
        print_error "Evals API server failed to start. Checking for errors..."

        # Check if process is still running
        if ! kill -0 "$api_pid" 2>/dev/null; then
            print_error "Evals API process has died. Checking logs for startup errors:"
        else
            print_error "Evals API process is running but not responding. Checking logs:"
        fi

        if [[ -f "$RF_LOG_PATH/evals_api.log" ]]; then
            echo "=== Last 30 lines of evals_api.log ==="
            tail -30 "$RF_LOG_PATH/evals_api.log"
            echo "=== End of logs ==="
            echo ""

            # Look for specific error patterns
            if grep -q "Error\|Exception\|Traceback\|Failed\|ImportError\|ModuleNotFoundError" "$RF_LOG_PATH/evals_api.log"; then
                print_error "Found error messages in logs:"
                grep -A 5 -B 2 "Error\|Exception\|Traceback\|Failed\|ImportError\|ModuleNotFoundError" "$RF_LOG_PATH/evals_api.log" | head -20
            fi
        else
            print_error "No evals_api.log file found"
        fi

        # Check if there are any Python errors in the process
        if kill -0 "$api_pid" 2>/dev/null; then
            print_status "Evals API process details:"
...
# Function to start frontend server (reuses fit frontend)
start_frontend() {
    print_status "Starting frontend tracking server for Evals..."

    if ! check_port $RF_FRONTEND_PORT "Frontend server"; then
        return 1
    fi

    # Check if frontend directory exists (using fit frontend)
...
show_status() {
    print_status "RapidFire AI Evals Services Status:"
    echo "=================================="
    ...
    if [[ "$RF_COLAB_MODE" == "true" ]]; then
        print_success "RapidFire Evals running in Colab mode!"
        ...
        print_success "RapidFire Evals Frontend is ready!"
    ...
    # Always check evals_api.log
    if [[ -f "$RF_LOG_PATH/evals_api.log" ]]; then
        local size=$(du -h "$RF_LOG_PATH/evals_api.log" | cut -f1)
        print_status "- $RF_LOG_PATH/evals_api.log: $size"
    else
        print_warning "- $RF_LOG_PATH/evals_api.log: not found"
    fi
...
start_services() {
    ...
    print_status "Starting $total_services Evals service(s)..."
    ...
    if start_api_server; then
        ...
        print_error "Failed to start Evals API server"
    fi
...
# Main execution
main() {
    print_status "Starting RapidFire AI Evals services..."
    ...
    if start_services; then
        print_success "All Evals services started successfully!"
        show_status
        ...
        print_error "Failed to start one or more Evals services"

        # Show summary of all log files for debugging
        print_status "=== Startup Failure Summary ==="
        for log_file in "mlflow.log" "evals_api.log" "frontend.log"; do
...
case "${1:-start}" in
    ...
    *)
        ...
        echo "  start   - Start all Evals services (default)"
        echo "  stop    - Stop all Evals services"
        echo "  status  - Show service status"
        echo "  restart - Restart all Evals services"
