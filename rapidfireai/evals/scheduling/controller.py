                    ...
                    if metric_run_id:
                        ...
                        for metric_name, metric_data in ordered_metrics.items():
                            ...
                            if lower_bound is not None and isinstance(lower_bound, (int, float)):
                                try:
                                    self.metric_manager.log_metric(
                                        metric_run_id, f"{metric_name}_lower_bound", float(lower_bound), step=step
                                    )
                                except Exception as e:
                                    self.logger.debug(
                                        f"Failed to log final metric {metric_name}_lower_bound to MetricLogger: {e}"
                                    )
                            ...
                            if upper_bound is not None and isinstance(upper_bound, (int, float)):
                                try:
                                    self.metric_manager.log_metric(
                                        metric_run_id, f"{metric_name}_upper_bound", float(upper_bound), step=step
                                    )
                                except Exception as e:
                                    self.logger.debug(
                                        f"Failed to log final metric {metric_name}_upper_bound to MetricLogger: {e}"
                                    )
                                        ...
                                        for metric_name, metric_data in metrics_with_ci.items():
                                            ...
                                            if lower_bound is not None and isinstance(lower_bound, (int, float)):
                                                try:
                                                    ...
                                                    self.logger.debug(
                                                        f"Failed to log metric {metric_name}_lower_bound to MetricLogger: {e}"
                                                    )
                                            ...
                                            if upper_bound is not None and isinstance(upper_bound, (int, float)):
                                                try:
                                                    ...
                                                    self.logger.debug(
                                                        f"Failed to log metric {metric_name}_upper_bound to MetricLogger: {e}"
                                                    )
