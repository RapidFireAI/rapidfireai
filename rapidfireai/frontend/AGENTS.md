# AGENTS.md - MLflow Frontend Development

This file provides guidance to AI coding agents when working with the MLflow frontend code in this directory.
...
For testing UI changes in a real browser, AI coding agents can use the Playwright MCP (Model Context Protocol) integration.
...
If Playwright MCP is not available and you need to test UI changes, install it via your AI tool's MCP configuration. The MCP server itself is `npx @playwright/mcp@latest` over `stdio` — no env vars or auth required. Restart the tool (or reload the window / refresh the agent panel) after configuring so the new server is picked up.
...
Add to `~/.cursor/mcp.json` (user-scope) or `.cursor/mcp.json` at the project root (project-scope):
...
Run the Command Palette action **MCP: Add Server**, or add to `.vscode/mcp.json` at the workspace root:
...
Use the Cascade settings UI (**Manage MCPs**), or add to `~/.codeium/windsurf/mcp_config.json`:
...
MCP support varies by version; consult the tool's official MCP integration docs for the current setup pattern. The same `npx @playwright/mcp@latest` server command applies once a config slot is available.
