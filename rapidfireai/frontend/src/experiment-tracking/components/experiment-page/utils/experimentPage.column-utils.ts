...
const RUN_NAME_COLUMN_WIDTH = 105;
...
export const getFrameworkComponents = () => ({
  ...
  // runStatusCellRenderer: RunStatusCellRenderer,
...
export const useRunsColumnDefinitions = ({
  ...
  onOpenController,
}: UseRunsColumnDefinitionsParams & { onOpenController?: (runUuid: string, runName: string) => void }) => {
  ...
  const columnSet = useMemo(() => {
    ...
    // TODO: Add in later, but need to pass experiment name + run ID to dispatcher since run IDs are not globally unique
    // Add Run Status column
    // columns.push({
