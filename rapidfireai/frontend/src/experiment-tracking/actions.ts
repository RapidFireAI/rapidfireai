...
export const SET_PROXY_TARGET = 'SET_PROXY_TARGET';
export const UPDATE_MLFLOW_SERVER_STATUS = 'UPDATE_MLFLOW_SERVER_STATUS';
export const SET_DISPATCHER_PROXY_TARGET = 'SET_DISPATCHER_PROXY_TARGET';
export const UPDATE_DISPATCHER_STATUS = 'UPDATE_DISPATCHER_STATUS';

export const setMlFlowProxyTarget = (params: { target: string, userId: string }) => async (dispatch: Dispatch) => {
  const { target, userId } = params;
  try {
    const response = await fetch(`/set-main-proxy?target=${encodeURIComponent(target)}`, {
      headers: {
        'x-user-id': userId
      }
    });
    ...
    dispatch({ type: SET_PROXY_TARGET, payload: target });
  } catch (error) {
    console.error('Failed to set proxy target:', error);
  }
};

export const updateMLflowServerStatus = (status: { isValid: boolean, uri: string | null }) => ({
  type: UPDATE_MLFLOW_SERVER_STATUS,
  payload: status,
});

export const setDispatcherProxyTarget = (params: { target: string, userId: string }) => async (dispatch: Dispatch) => {
  const { target, userId } = params;
  try {
    const response = await fetch(`/set-dispatcher-proxy?target=${encodeURIComponent(target)}`, {
      headers: {
        'x-user-id': userId
      }
    });
    ...
    dispatch({ type: SET_DISPATCHER_PROXY_TARGET, payload: target });
  } catch (error) {
    console.error('Failed to set dispatcher proxy target:', error);
  }
};
