...
from rapidfireai.fit.ml.callbacks import (
...
def create_rf_trainer(
    trainer_type: str, trainer_config=None, shm_manager=None, use_fsdp=False, **kwargs
):
    """
    ...
    class RFTrainer(base_trainer_class):
        ...
        def __init__(
            self, trainer_config=None, shm_manager=None, use_fsdp=False, **kwargs
        ):
        ...
        def restore_optimizer_state(self):
            ...
            try:
                if (
                    ...
                    device_optimizer_state = move_tensors_to_device(
            ...
            with ctx:
                if fsdp_plugin.state_dict_type == StateDictType.FULL_STATE_DICT:
                    ...
                    flattened_osd = FSDP.optim_state_dict_to_load(
                        model=model, optim=self.optimizer, optim_state_dict=optim_state
                    )
        ...
        def restore_scheduler_state(self):
            ...
            try:
                if (
                    ...
                    device_scheduler_state = move_tensors_to_device(
                        scheduler_state, device
                    )
        ...
        def _load_optimizer_and_scheduler(self, resume_from_checkpoint: str | None):
            ...
            if (
                self.trainer_config.completed_steps > 0
                or self.trainer_config.warm_started_from is not None
            ):
                if self.use_fsdp:
                    self.restore_fsdp_optimizer_state(
                        rank=self.trainer_config.local_rank
                    )
                else:
            ...
            if is_peft_model(self.model):
                ...
                if self.is_fsdp_enabled:
                    ...
                    with FSDP.summon_full_params(self.model, writeback=False):
                        ...
                        fsdp_version = (
                            getattr(fsdp_plugin, "fsdp_version", 1)
                            if fsdp_plugin
                            else 1
                        )
...
def create_trainer_instance(
    ...
    training_args, global_step_args = _configure_training_args(
        training_args, trainer_config, use_fsdp=use_fsdp
    )
    ...
    if use_shared_memory:
        ...
        model_instance, tokenizer = load_checkpoint_from_disk(
            trainer_config, device, is_peft=is_peft
        )
    ...
    if trainer_config.completed_steps > 0:
        if use_shared_memory:
            trainer = restore_trainer_from_shared_memory(
                trainer, trainer_config, shm_manager, use_fsdp=use_fsdp
            )
        else:
