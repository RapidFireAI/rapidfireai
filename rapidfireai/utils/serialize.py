"""
RapidFire AI Serialization

Serialization utilities for storing complex objects in the database.
"""
...
def encode_payload(payload: object) -> str:
    """
    Encode a Python object for database storage using dill.

    Args:
        payload: Any Python object (including functions, classes, etc.)
...
def decode_db_payload(payload: str) -> object:
    """
    Decode a database payload back to a Python object.

    Args:
        payload: Base64-encoded string from database
...
def extract_pipeline_config_json(pipeline_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if "pipeline" in pipeline_config:
        ...
        # Try to import the model config classes for type checking
        try:
            ...
            # Model config classes not available, try generic extraction
            if hasattr(pipeline, "__class__"):
                class_name = pipeline.__class__.__name__.lower()
                if "vllm" in class_name:
                    json_config["pipeline_type"] = "vllm"
...
__all__ = ["encode_payload", "decode_db_payload", "extract_pipeline_config_json"]
