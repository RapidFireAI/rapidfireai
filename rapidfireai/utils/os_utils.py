    ...
    try:
        if dist_id in ["ubuntu", "debian"]:
            ...
            result = subprocess.run(
                ["dpkg-query", "-W", "-f=${Package}\n", package_pattern], capture_output=True, text=True, check=False
            )
            ...
            result = subprocess.run(["rpm", "-qa", package_pattern], capture_output=True, text=True, check=False)
            if result.returncode == 0:
                return [pkg.strip() for pkg in result.stdout.strip().split("\n") if pkg.strip()]
            return []
            ...
            for cmd in [["dpkg-query", "-W", "-f=${Package}\n", package_pattern], ["rpm", "-qa", package_pattern]]:
                try:
