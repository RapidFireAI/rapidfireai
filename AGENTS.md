...
- [`docs/AGENTS.md`](docs/AGENTS.md) — **end-user-facing** install & setup guide, read by AI agents helping a user install or use the package. *Not* for contributors. Placed under `docs/` so maintainer auto-loaders do not pick it up as project rules. Update triggers are listed in [Keeping `docs/AGENTS.md` in sync](#keeping-docsagentsmd-in-sync) below.

### Scope guard

If your task is "install rapidfireai for user X" or "set up rapidfireai in a user's project," you are in the wrong place — you should be inside the user's project, not this repo. Being inside this repo means you are doing maintenance work on `rapidfireai` itself, and this file is correct for that. If you somehow loaded this as project rules while assisting an end user, fetch [`docs/AGENTS.md`](docs/AGENTS.md) instead.
...
`docs/AGENTS.md` is an end-user-facing operational guide read by AI coding agents (Claude Code, Cursor, Codex, GitHub Copilot, Windsurf, Aider, Junie) when they help a user install or set up `rapidfireai`. It is **not** for contributors — for repo-internal architecture, build, and contributor flow, *this* file (`AGENTS.md`) is authoritative.
...
`docs/AGENTS.md` *does* inline the stable layer the README does not own. **Whenever you change any of the following, update the corresponding part of `docs/AGENTS.md` in the same PR** — but only where the guide actually covers that area (it may be scoped to install/setup, or extended to also cover code integration):

- **Public API of `rapidfireai.Experiment`** — the `__init__` signature (especially the `mode` parameter), or any of `run_fit`, `run_evals`, `get_results`, `cancel_current`, `end`, `get_log_file_path`.
- **Top-level exports** in `rapidfireai/__init__.py` or **AutoML exports** in `rapidfireai/automl/__init__.py` (e.g., `RFGridSearch`, `RFRandomSearch`, `List`, `Range`, `RFModelConfig`, `RFLoraConfig`, `RFSFTConfig`, `RFDPOConfig`, `RFGRPOConfig`, `RFvLLMModelConfig`, `RFAPIModelConfig`, `RFLangChainRagSpec`, `RFPromptManager`).
- **Trainer-type taxonomy** in `rapidfireai/fit/ml/trainer.py` — the `trainer_type` → trainer-class mapping (currently `SFT` / `DPO` / `GRPO`).
- **User-supplied callback signatures** — `create_model_fn`, `formatting_func`, `compute_metrics`, `preprocess_fn`, `postprocess_fn`, `compute_metrics_fn`, `accumulate_metrics_fn`.
- **CLI subcommand structure** — adding, renaming, or removing `init`, `init --evals`, `start`, `stop`, `doctor`, `jupyter`, `--version`, or changing the dependency-set choice between fine-tuning and RAG/eval.
- **Tutorial taxonomy** — the lite/full split, the OpenAI-vs-self-hosted classification, or filenames under `tutorial_notebooks/{fine-tuning,post-training,rag-contexteng}/` referenced in the guide's tutorials table.
- **README section headings** — if you rename `§Prerequisites`, `§Install and Get Started`, or `§Troubleshooting` in `README.md`, update the cross-references in `docs/AGENTS.md` so they still resolve.
